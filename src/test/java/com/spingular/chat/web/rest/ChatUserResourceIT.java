...
public class ChatUserResourceIT {
    ...
    public void getAllChatUsersWithEagerRelationshipsIsEnabled() throws Exception {
        ChatUserResource chatUserResource = new ChatUserResource(chatUserServiceMock, chatUserQueryService);
        when(chatUserServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restChatUserMockMvc = MockMvcBuilders.standaloneSetup(chatUserResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restChatUserMockMvc.perform(get("/api/chat-users?eagerload=true"))
        .andExpect(status().isOk());

        verify(chatUserServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllChatUsersWithEagerRelationshipsIsNotEnabled() throws Exception {
        ChatUserResource chatUserResource = new ChatUserResource(chatUserServiceMock, chatUserQueryService);
            when(chatUserServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restChatUserMockMvc = MockMvcBuilders.standaloneSetup(chatUserResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
