...
public class ChatUserResource {
    ...
    public ResponseEntity<List<ChatUserDTO>> getAllChatUsers(ChatUserCriteria criteria) {
        log.debug("REST request to get ChatUsers by criteria: {}", criteria);
        List<ChatUserDTO> entityList = chatUserQueryService.findByCriteria(criteria);
